...
# Funciones para implementar y evaluar modelos de regrsin multipleó en R
# Rub[en Pizarro Gurrola
# Abril 2025
...
f_evaluacion <- function(modelos, datos_validacion, variable_dependiente, nombres = NULL) {
  
  #------------------------------------------
  # Normalizar entrada
  #------------------------------------------
  if (inherits(modelos, "lm")) {
    modelos <- list(modelos)
  }
  
  if (!is.list(modelos)) {
    stop("modelos debe ser un modelo lm o lista de modelos")
  }
  ...
  for (m in modelos) {
    if (!inherits(m, "lm")) {
      stop("Uno de los elementos no es un modelo lm válido")
    }
  }
  ...
  for (i in seq_along(modelos)) {
    ...
    resultados <- rbind(resultados, data.frame(
      Modelo = nombres[i],
      R_square = round(resumen$r.squared, 4),
      R_square_ajustado = round(resumen$adj.r.squared, 4),
      MSE = round(mse, 4),
      RMSE = round(rmse, 4),
      MAE = round(mae, 4)
    ))

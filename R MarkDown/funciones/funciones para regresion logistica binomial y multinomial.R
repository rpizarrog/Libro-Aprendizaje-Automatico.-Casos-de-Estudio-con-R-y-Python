...
# Junio 2026
# Funciones para regresin log[istica binaria
...
f_matriz_confusion <- function(modelo, datos_validacion, variable_dependiente, clase_interes = NULL){
  ...
  y_real <- factor(datos_validacion[[variable_dependiente]])
  ...
  if(inherits(modelo, "glm")){
    probabilidades <- predict(modelo, newdata = datos_validacion, type = "response")
    
    clases <- levels(y_real)
    pred <- factor(ifelse(probabilidades >= 0.50, clases[2], clases[1]), levels = clases)
  }
  
  else if(inherits(modelo, "multinom")){
    
    pred <- predict(modelo, newdata = datos_validacion, type = "class")
  ...
  if(length(levels(y_real)) == 2){
    
    if(is.null(clase_interes)){clase_interes <- levels(y_real)[2]}
    
    mc <- confusionMatrix(data = pred, reference = y_real, positive = clase_interes)
  ...
  #-------------------------------------------------------
    ...
    mc <- confusionMatrix(data = pred, reference = y_real)
    }
...
f_evaluar_modelos <- function(
  ...
  #-------------------------------------------------------
  # CONVERTIR MODELO INDIVIDUAL A LISTA
  #-------------------------------------------------------
  ...
  if(
    inherits(datos_validacion,"data.frame")
  ){

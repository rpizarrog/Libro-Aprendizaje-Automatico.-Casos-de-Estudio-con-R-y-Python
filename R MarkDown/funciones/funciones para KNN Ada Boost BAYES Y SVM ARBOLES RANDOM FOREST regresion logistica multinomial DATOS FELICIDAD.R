...
# Junio 2026
# Funcione para SVMarboles de regresión y random florest
...
#=========================================================
# FUNCIÓN
# f_estandarizar_validacion()
#=========================================================

f_estandarizar_validacion <- function(
...
#
# Devuelve:
#   Objeto con toda la información necesaria para
#   realizar predicciones mediante KNN
#
# Autor:
#   Rubén Pizarro Gurrola
#========================================================

f_crear_modelo_KNN <- function(
    datos_entrenamiento,
    variable_dependiente,
    k = 5,
    escalar = FALSE
  ...
  if(!is.data.frame(datos_entrenamiento)){
    
    stop("datos_entrenamiento debe ser un data.frame.")
  }
  
  
  if(!(variable_dependiente %in%
       names(datos_entrenamiento))){
    
    stop("La variable dependiente no existe.")
  }
  
  
  if(k <= 0){
    
    stop("k debe ser mayor que cero.")
  }
  ...
  if(any(!sapply(X,is.numeric))){
    ...
    stop(
      "KNN requiere variables predictoras numéricas."
    )
  }
  ...
  cat("\n")
  cat("---------------------------------------\n")
  cat("Modelo KNN construido correctamente\n")
  cat("---------------------------------------\n")
  cat("Observaciones :",nrow(X),"\n")
  cat("Variables     :",ncol(X),"\n")
  cat("k             :",k,"\n")
  cat("Escalamiento  :",escalar,"\n")
  cat("---------------------------------------\n\n")
...
f_crear_modelo_Ada_Boost <- function(
    datos_entrenamiento,
    variable_dependiente,
    mfinal = 50,
    boos = TRUE){
  ...
  if(!is.data.frame(datos_entrenamiento)){
    ...
    stop(
      "datos_entrenamiento debe ser un data.frame."
    )
  ...
  cat("\n")
  cat("------------------------------------------\n")
  cat("Modelo AdaBoost construido correctamente\n")
  cat("------------------------------------------\n")
  cat(
    "Observaciones :",
...
f_crear_modelo_Bayes <- function(
  ...
  cat("\n")
  cat("------------------------------------------\n")
  cat("Modelo Naïve Bayes construido correctamente\n")
  cat("------------------------------------------\n")
  cat(
    "Observaciones :",
    nrow(datos_entrenamiento),
  ...
  cat(
    "Clases        :",
    length(levels(datos_entrenamiento[[variable_dependiente]])),
    "\n"
  )
  cat("------------------------------------------\n\n")
...
f_crear_modelo_SVM_lineal <- function(
...
f_crear_modelo_SVM_polinomial <- function(
...
f_crear_modelo_SVM_radial <- function(
  datos,
  variable_dependiente,
  C = 0.1,
  gamma = NULL){
...
f_predecir_KNN <- function(
  ...
  if(!inherits(modelo,"modelo_KNN")){
    
    stop(
      "El objeto no corresponde a un modelo KNN."
    )
  ...
  variables <- names(modelo$X)
  
  if(!all(variables %in% names(datos_validacion))){
    
    stop(
      "Los datos de validación no contienen todas las variables predictoras."
    )
    
  }
  
  X_validacion <- datos_validacion[
    ,
    variables,
    drop = FALSE
  ]
  ...
  if(any(!sapply(X_validacion,is.numeric))){
...
f_matriz_confusion <- function(
  ...
  else if(inherits(modelo,"modelo_KNN")){
  ...
  else if(inherits(modelo,"boosting")){
  ...
  else if(inherits(modelo,"naiveBayes")){
...
f_evaluar_modelos <- function(
  ...
  if(is.data.frame(datos_validacion)){
    
    datos_validacion <- rep(

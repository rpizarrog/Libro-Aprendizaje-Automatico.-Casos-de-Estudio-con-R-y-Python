...
# Se adecúan para Múltiple Polinomial
...
f_multiple_polinomial <- function(datos, variable_dependiente, orden = 1, ver_resumen = TRUE) {
  #------------------------------------------------------------
  # f_multiple_polinomial()
  #
  # Objetivo:
  #   Construir modelo polinomial múltiple SIN redundancias
  #
  # Evita:
  #   - coeficientes NA
  #   - multicolinealidad perfecta por duplicación
  #------------------------------------------------------------
  ...
  if (orden < 1) {
    stop("El orden debe ser >= 1")
  }
  
  #------------------------------------------
  # Variables independientes
  #------------------------------------------
  variables <- setdiff(names(datos), variable_dependiente)
  
  #------------------------------------------
  # 1. Términos polinomiales (SIN x^1 duplicado)
  #------------------------------------------
  if (orden == 1) {
    terminos_poly <- variables
  } else {
    terminos_poly <- unlist(lapply(variables, function(var) {
      c(
        var,                                   # término lineal
        paste0("I(", var, "^", 2:orden, ")")   # potencias >= 2
      )
    }))
  }
  
  #------------------------------------------
  # 2. Interacciones (hasta orden especificado)
  #------------------------------------------
  ...
  #------------------------------------------
  cat("\n============================\n")
  cat("Modelo Polinomial de Orden", orden, "\n")
  cat("============================\n")
  cat("Variables independientes:", length(variables), "\n")
  cat("Número de términos:", length(attr(model.matrix(modelo), "dimnames")[[2]]), "\n")
...
f_evaluacion <- function(modelos, datos_validacion_list, variable_dependiente, 
  ...
  if (!is.list(datos_validacion_list)) datos_validacion_list <- list(datos_validacion_list)
  ...
  for (i in seq_along(modelos)) {
    ...
    #------------------------------------------
    # MODELOS lm (incluye polinomiales)
    #------------------------------------------
    if (inherits(modelo, "lm")) {
      
      # 🔥 robusto para polinomiales
      X_val <- model.matrix(formula(modelo), datos_validacion)
    ...
    #------------------------------------------
    # MODELOS glmnet (LASSO / RIDGE)
    #------------------------------------------
    else if ("glmnet" %in% class(modelo)) {
      
      if (is.null(lambdas) || is.na(lambdas[i])) {
        stop(paste("Falta lambda para el modelo:", nombres[i]))
      }

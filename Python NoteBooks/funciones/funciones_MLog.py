# FUNCIONES PARA MODELOS DE REGRESIÓN LINEAL, POLINOMIAL y LOGARITMICA
# Archivo: funciones_MLog.py
...
def f_ecuaciones_modelos(modelos, nombres_modelos=None):
    ...
    for modelo, nombre in zip(modelos, nombres_modelos):
        ...
        #--------------------------------------------------------
        # CASO 1: Modelo tipo Pipeline (polinomial)
        #--------------------------------------------------------
        if hasattr(modelo, "named_steps"):
            
            if "poly" in modelo.named_steps:
                ...
                # Pipeline sin poly
                lr = list(modelo.named_steps.values())[-1]
                coeficientes = lr.coef_
                intercepto = lr.intercept_
                nombres_vars = [f"X{i}" for i in range(len(coeficientes))]
        
        #--------------------------------------------------------
        # CASO 2: statsmodels (log, lineal, etc.)
        #--------------------------------------------------------
        else:
            params = modelo.params
            intercepto = params[0]
            coeficientes = params[1:]
        ...
        for c, nombre_var in zip(coeficientes, nombres_vars):
            ...
            # limpiar nombres (más bonito para libro)
            nombre_var = str(nombre_var)
            nombre_var = nombre_var.replace("np.log", "ln")
